...
then
  if consul-template -consul-addr=http://consul-priv.dev.articulate.zone -vault-addr=http://vault-priv.dev.articulate.zone -template=/consul-template/${APP_ENV}/export-vault.ctmpl:/tmp/export-vault.sh -once -max-stale=0
  then
